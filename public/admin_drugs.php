        ...
        <main class="flex-1 min-w-0 ml-[240px] p-6 lg:p-8 flex flex-col gap-6">
            ...
            <section class="relative rounded-[24px] bg-white/[0.03] border border-white/[0.06] backdrop-blur-2xl p-6 overflow-hidden flex justify-between items-center flex-wrap gap-4">
                <div>
                    <h2 class="text-white font-semibold text-xl flex items-center gap-2" style="font-family: 'Space Grotesk', sans-serif;">
                        <i data-lucide="pill" class="text-blue-300 w-5 h-5"></i>
                        <span>Quản lý thực thể Thuốc</span>
                    </h2>
                    <p class="text-white/50 text-sm mt-1" style="font-family: 'Inter', sans-serif;">
                        Thêm mới, xóa và theo dõi danh sách thực thể thuốc trong cơ sở dữ liệu.
                    </p>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
                ...
                <div class="lg:col-span-2 rounded-[24px] bg-white/[0.03] border border-white/[0.06] p-6 backdrop-blur-2xl">
                    <h3 class="text-white font-semibold text-[15px] mb-2" style="font-family: 'Space Grotesk', sans-serif;">Thêm thuốc mới</h3>
                    <p class="text-white/40 text-xs mb-5" style="font-family: 'Inter', sans-serif;">Nhập mã định danh thuốc và đặc trưng hóa học (SMILES).</p>
                    
                    <form method="post" class="space-y-4">
                        <input type="hidden" name="action" value="create">
                        
                        <label class="flex flex-col gap-1.5">
                            <span class="text-white/50 px-1 text-[11px] font-semibold uppercase tracking-wider" style="font-family: 'Inter', sans-serif;">Mã thuốc (Drug ID)</span>
                            <input class="w-full h-11 px-4 rounded-xl bg-black/40 border border-white/[0.08] focus:border-blue-400/40 focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-white placeholder:text-white/30 text-sm" name="source_code" placeholder="Ví dụ: DB00014" required style="font-family: 'Inter', sans-serif;">
                        </label>
                        
                        <label class="flex flex-col gap-1.5">
                            <span class="text-white/50 px-1 text-[11px] font-semibold uppercase tracking-wider" style="font-family: 'Inter', sans-serif;">Tên thuốc</span>
                            <input class="w-full h-11 px-4 rounded-xl bg-black/40 border border-white/[0.08] focus:border-blue-400/40 focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-white placeholder:text-white/30 text-sm" name="name" placeholder="Ví dụ: Goserelin" required style="font-family: 'Inter', sans-serif;">
                        </label>
                        
                        <label class="flex flex-col gap-1.5">
                            <span class="text-white/50 px-1 text-[11px] font-semibold uppercase tracking-wider" style="font-family: 'Inter', sans-serif;">Cấu trúc SMILES</span>
                            <textarea class="w-full h-24 p-3 rounded-xl bg-black/40 border border-white/[0.08] focus:border-blue-400/40 focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-white placeholder:text-white/30 text-sm" name="smiles" placeholder="Nhập chuỗi SMILES biểu diễn cấu trúc thuốc..." style="font-family: 'Inter', sans-serif;"></textarea>
                        </label>
                        
                        <label class="flex flex-col gap-1.5">
                            <span class="text-white/50 px-1 text-[11px] font-semibold uppercase tracking-wider" style="font-family: 'Inter', sans-serif;">Mô tả chi tiết</span>
                            <textarea class="w-full h-24 p-3 rounded-xl bg-black/40 border border-white/[0.08] focus:border-blue-400/40 focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-white placeholder:text-white/30 text-sm" name="description" placeholder="Thông tin chi tiết về thuốc..." style="font-family: 'Inter', sans-serif;"></textarea>
                        </label>
                ...
                <div class="lg:col-span-3 rounded-[24px] bg-white/[0.03] border border-white/[0.06] p-6 backdrop-blur-2xl">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            <h3 class="text-white font-semibold text-[15px]" style="font-family: 'Space Grotesk', sans-serif;">Danh sách thuốc mới</h3>
                            <p class="text-white/40 text-xs" style="font-family: 'Inter', sans-serif;">Hiển thị tối đa 50 thực thể thuốc vừa thêm.</p>
                        </div>
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-300 text-[11px] font-semibold tracking-wider font-mono">
                            <?= count($rows) ?> bản ghi
                        </span>
                    </div>
                            ...
                            <tbody>
                                <?php if (empty($rows)): ?>
                                    <tr>
                                        <td colspan="4" class="px-4 py-8 text-center text-white/35" style="font-family: 'Inter', sans-serif;">
                                            Chưa có dữ liệu thuốc nào được lưu.
                                        </td>
                                    </tr>
                                <?php endif; ?>
                                <?php foreach ($rows as $row): ?>
                                    <tr class="border-b border-white/[0.04] hover:bg-white/[0.02] transition-all">
                                        ...
                                        <td class="px-4 py-3">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-300 font-mono text-[10px] font-semibold uppercase">
                                                <?= e((string) $row['source_code']) ?>
                                            </span>
                                        </td>
                                        <td class="px-4 py-3 font-semibold text-white/80"><?= e((string) $row['name']) ?></td>
                                        <td class="px-4 py-3 text-right">
                                            <form method="post" class="inline-block" onsubmit="return confirm('Bạn có chắc chắn muốn xoá thuốc này?');">
                                                <input type="hidden" name="action" value="delete">
                                                <input type="hidden" name="id" value="<?= e((string) $row['id']) ?>">
                                                <button class="inline-flex items-center justify-center w-7 h-7 rounded-lg text-white/40 hover:text-red-400 hover:bg-white/[0.05] transition-all" type="submit" title="Xóa thuốc">
